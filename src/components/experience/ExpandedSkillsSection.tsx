...
const ExpandedSkillsSection = ({ lang }: { lang?: Locale }) => {
  ...
  const skillCategories = [
    {
      name: dictionary?.skills?.categories?.programmingLanguages || "Programming Languages",
      icon: <Code className="h-6 w-6 text-indigo-600" />,
      skills: ["C#", "JavaScript", "HTML", "CSS", "SQL", "JAVA", "Bash"] as Skill[]
    },
    {
      name: dictionary?.skills?.categories?.frameworksLibraries || "Frameworks & Libraries",
    ...
    {
      name: dictionary?.skills?.categories?.databases || "Databases",
      icon: <Database className="h-6 w-6 text-indigo-600" />,
      skills: ["Microsoft SQL Server", "PostgreSQL", "Oracle SQL"] as Skill[]
    },
    {
      name: dictionary?.skills?.categories?.cloudInfrastructure || "Cloud & Infrastructure",
    ...
    {
      name: dictionary?.skills?.categories?.toolsEnvironments || "Tools & Environments",
    ...
    {
      name: dictionary?.skills?.categories?.languages || "Languages",
      icon: <Globe className="h-6 w-6 text-indigo-600" />,
      skills: [
        { 
          name: "Estonian", 
          level: dictionary?.skills?.languageLevels?.native || "Native" 
        },
        { 
          name: "English", 
          level: dictionary?.skills?.languageLevels?.professional || "Professional" 
        }
      ] as LanguageSkill[]
    }
  ];
          ...
          <div className="grid grid-cols-1 gap-y-10 gap-x-8 lg:grid-cols-2 xl:grid-cols-3">
            {skillCategories.map((category, index) => (
              <div key={index} className="bg-white p-6 rounded-lg shadow-md border border-gray-100 h-full transition-all duration-200 hover:shadow-lg hover:-translate-y-1">
                ...
                {index === 5 ? (
                  // Only the Languages category (index 5) has language skills
                          ...
                          <div key={langIndex} className="flex flex-col">
                            ...
                            <div className="w-full bg-gray-200 rounded-full h-2">
                              <div 
                                ...
                                style={{ width: lang.level.includes("Native") || lang.level.includes("Emakeel") ? "100%" : "80%" }}
                              ></div>
                            </div>
                          </div>
                        );
